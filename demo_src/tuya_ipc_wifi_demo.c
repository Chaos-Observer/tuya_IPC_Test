...
{   
    printf("starting station_connect ...\n");
    station_connect = 1;
    if(sniffer_set_done)
    {
...
OPERATE_RET tuya_adapter_wifi_ap_start(IN CONST WF_AP_CFG_IF_S *cfg)
{
    ...
    char cmdName[128];
    memset(cmdName, 0, sizeof(cmdName));
    // snprintf(cmdName, sizeof(cmdName), "wifi_ap_opt.sh start %s %s %s &", WLAN_DEV, cfg->ssid, cfg->passwd);
    snprintf(cmdName, sizeof(cmdName), "./wifi_ap_opt.sh start %s %s %s &", WLAN_DEV, cfg->ssid, "12345678");
    // printf("exec wifi_ap_start : %s\r\n", cmdName);
    exec_cmd(cmdName);
    //Reserved
    return OPRT_OK;
}
...
if (pFile != NULL) {
    fprintf(pFile, "start		192.168.175.20\n");
    fprintf(pFile, "end		192.168.175.254\n");
    fprintf(pFile, "interface	%s\n", WLAN_DEV);
    fprintf(pFile, "option	subnet	255.255.255.0\n");

    fprintf(pFile, "opt	router	192.168.175.1\n");
...
memset(cmdName, 0, sizeof(cmdName));
snprintf(cmdName, sizeof(cmdName), "ifconfig %s 192.168.175.1 up", WLAN_DEV);
exec_cmd(cmdName);
memset(cmdName, 0, sizeof(cmdName));
